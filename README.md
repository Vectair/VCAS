...
**Distance reference**: a polar-plotted dot's screen position alone doesn't reliably communicate "how far away is that" the way AIR mode's real map landmarks do — without a scale cue, a genuinely distant aircraft can visually read as closer than it is. Both NAV styles now show dashed half-range/full-range rings (`UI.renderRangeRings()`) centred on the same anchor point the traffic plot uses, and every indicator's label carries its actual slant range (e.g. "8.4nm") alongside callsign/type — the same figure that drives where its dot is plotted, so the number and the position always agree.
...
- **Display & Accessibility** — Day/Auto/Night theme, colour-blind-safe palette toggle, NAV display style (Hybrid / Raw)
