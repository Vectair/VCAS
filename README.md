...
| `TOMTOM_API_KEY` | `""` | Optional, experimental — real-time-traffic-aware second routing provider (TomTom Orbis Routing API v2, driving only), selectable via a hidden dev-mode Settings toggle; blank disables it and ORS is used exclusively. See [Routing & Navigation Camera](#routing--navigation-camera) |
