...
- **No off-route detection or rerouting.** If you deviate from the active route, VCAS keeps guiding relative to the *original* polyline — `RouteGeometry.nearestOnLine()` always snaps to the nearest point on that line regardless of how far away you actually are, rather than detecting the deviation and requesting a fresh route from OpenRouteService. Distance-to-next-maneuver, the route line itself, and the guidance card's instruction all silently reference a route you may no longer be following. No workaround short of manually clearing and re-requesting the route.
