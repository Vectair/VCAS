...
| `TOMTOM_API_KEY` | `""` | Shared key for two optional TomTom integrations: (1) real-time-traffic-aware second routing provider (TomTom Orbis Routing API v2, driving only), experimental, selectable via a hidden dev-mode Settings toggle; (2) automatic destination-search fallback geocoder (TomTom Orbis Search API), always-on (no toggle) once a key is set, supplementing ORS/Pelias whenever it returns too few results. Blank disables both. See [Routing & Navigation Camera](#routing--navigation-camera) |
...
Optional — only needed if `DATA_PROVIDERS` includes `"adsb_exchange"` (not present in `config.js` by default):

| Key | Description |
|-----|-------------|
| `ADSB_API_KEY` | Required for the paid `"adsb_exchange"` provider |
| `ADSB_API_HOST` | Required for the paid `"adsb_exchange"` provider (e.g. `"adsbexchange.com"`) |
...
- Aircraft at/above 26,000 ft within 50 nm get a real contrail-formation check against today's conditions — see **Contrail-aware scoring** below.
...
**Contrail-aware scoring** (`src/logic/contrail.js`, `src/logic/upperAirProvider.js`): a high-flying jet's angular size alone often underrates it — the airframe may be a barely-resolvable dot, but its contrail is a bright, obvious streak. Aircraft at or above 26,000 ft and within 50 nm (identification range, not raw visibility — beyond that "they can still be seen but I couldn't definitively say they were from a certain aircraft") get checked against a real **Schmidt-Appleman criterion** (the standard exhaust-plume-mixing physics behind contrail-prediction tools, after Schumann 1996), using Open-Meteo's free, no-key pressure-level temperature/humidity forecast for today's actual conditions at that altitude — not just a flat assumption that any sufficiently-high aircraft has one. A persistent contrail (ice-supersaturated air, spreads into cirrus) floors the category at *Likely visible*; a forming-but-short-lived one floors it at *Possibly visible*; real conditions that don't support a contrail forming today apply no floor at all, same as any other aircraft. Never overrides a *better* angular-size result either way. Falls back to the original flat "always floor at *Possibly visible* if high/close enough" behaviour if no upper-air fetch has succeeded yet, so this is a strict improvement, never a regression. **Caveat**: `CONTRAIL_ENGINE_EFFICIENCY` (`src/logic/visibility.js`) is one generic constant shared across every aircraft — real propulsion efficiency varies by engine/thrust setting and isn't available from ADS-B — and the pressure-level fetch shares the same "not checked against a live response" caveat as the METAR/cloud-band fetches above.
...
**Position** (`Geo.projectToPolarPosition()`, `src/logic/geo.js`): NAV indicators are a true polar plot, not an edge frame — bearing maps to angle and distance maps to radius from an anchor point near the bottom of the screen (the same anchor the 3D camera itself uses), so closer traffic plots nearer to you and farther traffic plots farther out, rather than every aircraft regardless of range being jammed onto the frame edge. The radius scale is tied to the relevance teardrop's own dead-ahead range (`Relevance.DEFAULTS.rMaxNm`), so an indicator at the outer edge of the plot really is near the edge of relevance for that bearing. The icon itself (and its direction arrow) always stays exactly at that plotted point — nothing ever moves it. When two labels visibly overlap, `UI.declutterRenderedIndicators()` (`src/ui.js`) swings the LABEL boxes alone around their own fixed icons to separate them, the same way a real ND's tag deconfliction only relocates the data tag, never the target return itself.
...
- **Search by name/address** — `src/routing/activeGeocoder.js` (not `orsGeocoder.js` directly) queries [OpenRouteService's Geocoding API](https://openrouteservice.org/dev/#/api-docs/geocode/search/get) (Pelias-based; the same `CONFIG.ORS_API_KEY` already used for routing covers it too, no separate key needed) as the primary, always-queried source, debounced 350ms as you type (or immediately on Enter), biased toward your current position (`focus.point`)/`lat`+`lon`) so a same-named place near you outranks one across the country. **Automatically supplemented by TomTom's Orbis Search API** (`src/routing/tomtomGeocoder.js`) whenever ORS/Pelias comes back with fewer than 3 results — a real fix for named businesses (especially individual branches of national chains) that Pelias's OpenStreetMap-tagged POI data often has no entry for at all, even though street-address search on the same dataset is comparatively strong. Not a toggle or an experimental opt-in like the TomTom routing provider below — it's on automatically whenever `CONFIG.TOMTOM_API_KEY` is set, purely additive (ORS's own results are never replaced, only supplemented), and TomTom results within 120m of an existing ORS result are deduped rather than shown twice. Results render as a tappable list; picking one requests the route immediately, same as a map tap does.
- **Tap the map** — the next click/tap on the map itself supplies the target directly.

Either way, `requestRouteTo(lat, lon, label)` (`src/app.js`) requests a route from OpenRouteService to that point in the selected mode — a search result's real place name becomes the route card's destination label; a map tap falls back to raw coordinates, since it has nothing else to show. The route renders as a 3-layer glow/line/highlight polyline (`src/map.js`), with ETA, distance, and arrival clock shown in a bottom card — `_updateRouteCard()` recomputes all three live from the user's current snapped position on every GPS fix/render tick (`RouteGeometry.nearestOnLine()`/`distanceToIndex()` against the route's final coordinate, duration scaled proportionally against the route's own ORS-declared pace), not just written once at request time. A button on that card also toggles turn-by-turn text on/off, independent of the route line.
...
**Experimental second routing provider — TomTom (Orbis)** (`src/routing/activeRoutingProvider.js`): both `requestRouteTo()` and the reroute path above actually go through this dispatcher, not `orsProvider.js` directly. ORS is the permanent default and remains the automatic fallback any time TomTom is selected but a driving request fails. **Driving only** — TomTom's own Orbis Routing API v2 docs state `travelMode`'s only allowed value is `car`, so cycling/walking always use ORS directly regardless of the toggle, not just as a fallback. TomTom is off by default — set `CONFIG.TOMTOM_API_KEY` and flip the toggle in Settings → "Routing Provider (Experimental)" (only visible once dev mode is unlocked, same hidden 7-tap-the-brand-mark gesture as the VIEW/SPD developer panels) to try it. Its real, confirmed advantage is traffic-aware ETA/route geometry from TomTom's own live traffic data, which ORS's free tier doesn't provide (`_updateRouteCard()`'s ETA is otherwise a flat proportional scaling of ORS's own static-pace total, with zero real-time signal). Its known gap: no turn-by-turn steps yet (`tomtomProvider.js` always returns an empty `steps` array) — the guidance card falls back to the camera's own geometric turn detection, the same path ORS itself uses if its response is ever malformed, so this degrades gracefully rather than breaking, but street-name instructions won't appear while TomTom is active. Orbis Routing is also still in TomTom's own "public preview" status, per its docs — a real maturity caveat separate from cost, part of why this stays dev-gated.

**Destination-search fallback geocoder — also TomTom, but NOT dev-gated** (`src/routing/activeGeocoder.js`/`tomtomGeocoder.js`): a different shape of integration from the routing provider above, despite sharing a key/product family — this one is a real, always-on fix for a reported bug (VCAS's search couldn't find a real, popular national-chain daycare centre — nothing exotic), not an experimental opt-in. `_searchDestination()` (`app.js`) now calls `ActiveGeocoder.search()`, which always queries ORS/Pelias first and only additionally queries TomTom's Orbis Search API (`/maps/orbis/places/search/{query}.json` — fuzzy search over a licensed commercial POI dataset, not OSM-derived) when ORS comes back with fewer than 3 results. TomTom's own results are merged in (never replace ORS's), deduped against any ORS result within 120m so the same place doesn't appear twice. No Settings toggle exists for this — it's controlled entirely by whether `CONFIG.TOMTOM_API_KEY` is set (blank = ORS-only, unchanged from before this existed). The exact request/response shape and CORS support were confirmed directly from TomTom's own official, npm-published `@tomtom-org/maps-sdk` package source (`registry.npmjs.org` is reachable even though TomTom's own doc domains aren't from this project's sandbox) rather than guessed — see `tomtomGeocoder.js`'s own header comment for the full trail. Unlike the routing provider's CORS confirmation, this hasn't yet been checked against a live device response — worth a real on-device test once a key is filled in.
...
| ✦ | Visible — lights only (night / low visibility) |
...
**Why this exists:** angular size, METAR, Open-Meteo cloud-band, local-obstruction, and contrail-formation adjustments are all real now, but every one of them is an approximation on top of an approximation, and real spotting can still diverge from what the app predicts in ways the model can't currently explain (haze, precipitation, terrain occlusion — none modelled at all). This panel is how each of those gaps gets measured, contrail included — `visible_contrail` observations are exactly what a future calibration pass would check the new Schmidt-Appleman floor against.
...
/VCAS
  ...
  /src
    ...
    /logic
      ...
      visibility.js                 Angular size & detectability scoring (+ METAR/upper-air/local-obstruction/contrail adjustments)
      contrail.js                   Schmidt-Appleman contrail formation/persistence check (pure physics, no I/O)
      metarProvider.js              Nearest-METAR fetch/cache — feeds visibility.js only, no display surface
      upperAirProvider.js           Open-Meteo cloud-band + pressure-level temperature/humidity fetch/cache
      localObstruction.js           Building/wooded-landcover density from already-loaded map tiles
    ...
    /routing
      ...
      tomtomProvider.js             TomTom Orbis adapter — optional, experimental, real-time-traffic-aware, driving only
      activeRoutingProvider.js      Picks ORS vs TomTom by mode (hidden dev-mode toggle) — TomTom only for driving, falls back to ORS on failure
      orsGeocoder.js                OpenRouteService geocoding (destination search by name/address) — the default
      tomtomGeocoder.js             TomTom Orbis Search geocoding — supplementary, not experimental/toggled
      activeGeocoder.js             Queries ORS first, merges in TomTom results only when ORS returns too few
...
- **Visibility model** — flat terrain, daylight assumptions only; no terrain occlusion. Live METAR cloud-occlusion/reported-visibility, Open-Meteo cloud-band, local-obstruction, and now real contrail-formation adjustments all exist (see Visibility Categories above), but each is its own approximation — a METAR only describes conditions at the reporting station, Open-Meteo's cloud-band signal is regional-forecast-model resolution, and the contrail check's engine-efficiency constant is one generic value shared across every aircraft type. See the Ground-Truth Log Panel section above for how these gaps get measured/calibrated over time.
...
- **Reroute detection is a fixed distance threshold, not lane/road-aware.** `CONFIG.OFF_ROUTE_THRESHOLD_METERS` (50m) is a flat perpendicular-distance cutoff from the route polyline, the same for every road type and every transport mode — a motorway's own width could plausibly false-trigger on a tight-radius bend, and a footpath deviation of 50m is a much bigger relative miss than it is in a car. Untuned against real field data; see CLAUDE.md.
